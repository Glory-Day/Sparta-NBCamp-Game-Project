...
{
    public class MagicalDefenseDifferenceTextPresenter : PointDifferenceTextPresenter
    {
        public MagicalDefenseDifferenceTextPresenter(PointDifferenceTextView view, PlayerStatus model, Dispatcher dispatcher) : base(view, model, dispatcher)
        {
        }
        ...
        public override void Receive<T>(T message)
        {
            ...
            {
                var status = ((PlayerStatusData)playerStatus.data).MagicalDefense;
                switch (message)
                {
